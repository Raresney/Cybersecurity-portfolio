...
├── logs/
│   ├── attacker-commands.txt          # Commands typed by the attacker
│   ├── example-commands.txt           # Real-world botnet command patterns
│   ├── example-cowrie-session.json    # Raw Cowrie JSON log
│   ├── example-tty-log.txt           # TTY session replay output
│   └── tty-session.txt               # Full TTY session capture
├── screenshots/
│   ├── ss-listening-port22.jpg        # Port 22 listening confirmation
│   └── ssh-phone-connection.jpg       # SSH connection from mobile device
└── README.md
```
